...
func create_material(
	...
	if material is SpatialMaterial:
		print_debug("Spatial Material detected, using param \"%s\" to set albedo texture..." % SpatialMaterial.TEXTURE_ALBEDO)
		material.set_texture(SpatialMaterial.TEXTURE_ALBEDO, texture)
	if material is ShaderMaterial:
		var texture_param = "texture_albedo"
		print_debug("Shader Material detected, using param \"%s\" to set albedo texture..." % texture_param)
		material.set_shader_param(texture_param, texture)
		
		
	
	return material
	# Remove due to testing
