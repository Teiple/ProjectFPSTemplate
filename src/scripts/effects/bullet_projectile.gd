...
func set_up(attack_origin : AttackOriginInfo):
	global_position = attack_origin.fire_from
	...
	_origin_position = attack_origin.fire_from
	...
	Utils.lookat_direction(self, _direction)
...
func _check_and_collide() -> bool:
	...
	attack_result.collider = raycast.get_collider()
	attack_result.hit_direction = _direction
	attack_result.impact_force = _impact_force
	
	AttackResolver.resolve(attack_result)
	poolable_node_component.return_to_pool()
