...
func _ready():
	if start_state.is_empty():
		start_state = get_node("States").get_child(0).get_path()
	var blackboard = get_node("StateMachineBlackboard") as StateMachineBlackboard
	for state in get_node("States").get_children():
		state.connect("finished", self, "_change_state")
		if blackboard != null:
			state.set_blackboard(blackboard)
		states_map[get_path_to(state) as String] = state as StateMachineState
